...
//Get live user details
exports.liveUserDetails = async(req, res)=>{
  try{
    const liveUserId = req.query.id
    const liveUserDetails = await LiveUser.find({_id:liveUserId}).populate('liveUserId').lean()
    return res
        .status(200)
        .json({ status: true, message: "Success!!", liveUserDetails });
  }catch(error){
    console.log(error);
    return res.status(500).json({
      status: false,
